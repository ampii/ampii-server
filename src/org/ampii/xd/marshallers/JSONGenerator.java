    ...
    private void emitObject(Data data, boolean doingMetadata) throws XDException {
        ...
        if (context.canIncludeType() || data.isFromAny() || data.isFromNothing() || data.isDefinition()) {
            ...
            if (!localType.isEmpty()) { // emit local type if we are at the top level or it is not already implied
                if (context.cur_depth == 1 || data.isFromAny() || data.isFromNothing()) emitQuotedPair("$type", localType);
            }
            if (context.cur_depth == 1) { // at the top level, also output $effectiveType if applicable
                ...
                if (!effectiveType.isEmpty() && (!effectiveType.equals(localType))) emitQuotedPair("$effectiveType", effectiveType);
            }
        }
